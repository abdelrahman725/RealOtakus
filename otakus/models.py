...
def on_animes_to_review_change(sender, instance, **kwargs):
    ...
    if action == "post_remove":
        removed_animes = ", ".join(
            [
                cache.get("animes")[anime_id].anime_name
...
class Anime(base_models.Anime):
    ...
    def pending_contributions(self):
        return self.anime_questions.filter(
            approved__isnull=True, is_contribution=True
        ).count()
    ...
    def save(self, *args, **kwargs):
        if self.previously_active == False and self.active == True:
            create_notification(notification=self.anime_name, broad=True, kind="NA")
